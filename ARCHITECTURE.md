...
`resolveConfig()` returns a `ResolvedConfig` where every key wraps the value with resolution metadata (`{ value, source }`). This is necessary for `--show-config` display but leaks implementation detail into business logic.

**For code that only needs config values**, call `configValues(rc)` to strip metadata and get a plain `ConfigValues` object:

```ts
import { configValues } from "./config.ts";

const cv = configValues(resolvedConfig);
cv.baseBranch; // "main" — plain value, no .value accessor
```

**For tests**, use `makeTestConfig()` from `src/test-utils.ts` instead of duplicating `makeResolvedConfig()` helpers:

```ts
import { makeTestConfig } from "./test-utils.ts";

const cfg = makeTestConfig({ agentCommand: "echo", maxStuck: 1 });
cfg.baseBranch; // "main" — default preserved
```
