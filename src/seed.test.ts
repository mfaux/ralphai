...
describe("seed command", () => {
  ...
  it("seeds hello-ralphai into an initialized repo", () => {
    runCliOutput(["init", "--yes"], ctx.dir, testEnv());

    // init already creates hello-ralphai.md — remove it to test seed independently
    const { backlogDir } = getRepoPipelineDirs(ctx.dir, testEnv());
    const planPath = join(backlogDir, "hello-ralphai.md");
    ...
    expect(content).toContain("# Plan: Hello Ralphai");
    expect(output).toContain("Seeded hello-ralphai into backlog");
    expect(output).not.toContain("Cleaned");
  });

  it("overwrites existing hello-ralphai in backlog", () => {
    runCliOutput(["init", "--yes"], ctx.dir, testEnv());
    const { backlogDir } = getRepoPipelineDirs(ctx.dir, testEnv());
    const planPath = join(backlogDir, "hello-ralphai.md");
    ...
    const content = readFileSync(planPath, "utf8");
    expect(content).toContain("# Plan: Hello Ralphai");
    expect(output).toContain("Cleaned hello-ralphai from: backlog");
    expect(output).toContain("Seeded hello-ralphai into backlog");
  });
  ...
  it("cleans up in-progress plan and seeds fresh copy", () => {
    ...
    const slugDir = join(inProgressDir, "hello-ralphai");
    mkdirSync(slugDir, { recursive: true });
    writeFileSync(join(slugDir, "hello-ralphai.md"), "# in progress");
    ...
    const planPath = join(backlogDir, "hello-ralphai.md");
    expect(existsSync(planPath)).toBe(true);
    expect(readFileSync(planPath, "utf8")).toContain("# Plan: Hello Ralphai");
  ...
  it("cleans up archived plan and seeds fresh copy", () => {
    ...
    const backlogFile = join(backlogDir, "hello-ralphai.md");
    ...
    const archiveSlugDir = join(archiveDir, "hello-ralphai");
    mkdirSync(archiveSlugDir, { recursive: true });
    writeFileSync(join(archiveSlugDir, "hello-ralphai.md"), "# archived");
  ...
  it("cleans all stages when plan exists everywhere", () => {
    ...
    writeFileSync(join(backlogDir, "hello-ralphai.md"), "# backlog");

    // In-progress
    const ipDir = join(inProgressDir, "hello-ralphai");
    mkdirSync(ipDir, { recursive: true });
    writeFileSync(join(ipDir, "hello-ralphai.md"), "# wip");

    // Archive
    const archDir = join(archiveDir, "hello-ralphai");
    mkdirSync(archDir, { recursive: true });
    writeFileSync(join(archDir, "hello-ralphai.md"), "# done");
    ...
    expect(existsSync(join(backlogDir, "hello-ralphai.md"))).toBe(true);
  });
  ...
  it("aborts when runner PID is active", () => {
    ...
    const slugDir = join(inProgressDir, "hello-ralphai");
    mkdirSync(slugDir, { recursive: true });
    writeFileSync(join(slugDir, "hello-ralphai.md"), "# running");
  ...
  it("ignores stale PID file when process is gone", () => {
    ...
    const slugDir = join(inProgressDir, "hello-ralphai");
    mkdirSync(slugDir, { recursive: true });
    writeFileSync(join(slugDir, "hello-ralphai.md"), "# stale");
