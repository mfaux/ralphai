...
describe("uninstall command (repo-scoped default)", () => {
  ...
  it("uninstall --yes removes only this repo's state directory", async () => {
    ...
    const output = stripLogo(
      await runCliOutputInProcess(["uninstall", "--yes"], ctx.dir, testEnv()),
    );
  ...
  it("uninstall --yes prints not set up when config does not exist", async () => {
    const output = stripLogo(
      await runCliOutputInProcess(["uninstall", "--yes"], ctx.dir, testEnv()),
    );
  ...
  it("uninstall -y works as alias for --yes", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
    const configPath = getConfigFilePath(ctx.dir, testEnv());
    expect(existsSync(configPath)).toBe(true);

    const output = stripLogo(
      await runCliOutputInProcess(["uninstall", "-y"], ctx.dir, testEnv()),
    );
  ...
  it("uninstall --yes <target-dir> uninstalls from target directory", async () => {
...
describe("uninstall --global command", () => {
  ...
  it("uninstall --global --yes removes the global state directory", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
    const ralphaiHome = join(ctx.dir, ".ralphai-home");
    expect(existsSync(ralphaiHome)).toBe(true);
  ...
  it("uninstall --global --yes prints package manager uninstall command", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
  ...
  it("uninstall --global --yes handles missing global state directory", async () => {
    const output = stripLogo(
      await runCliOutputInProcess(
        ["uninstall", "--global", "--yes"],
        ctx.dir,
  ...
  it("uninstall --global --yes warns about plans in the backlog", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
  ...
  it("uninstall --global --yes warns about plans in progress", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
  ...
  it("uninstall --global --yes warns about backlog and in-progress plans together", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
  ...
  it("uninstall --global --yes does not warn when no plans exist", async () => {
    await runCliOutputInProcess(["init", "--yes"], ctx.dir, testEnv());
...
describe("uninstall help and flags", () => {
  ...
  it("uninstall --help shows usage, --global, and --yes flags", async () => {
  ...
  it("uninstall rejects unknown flags", async () => {
    const result = await runCliInProcess(
      ["uninstall", "--bogus"],
      ctx.dir,
      testEnv(),
