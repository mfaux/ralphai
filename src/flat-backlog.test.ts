...
describe("flat backlog plan discovery", () => {
  ...
  async function initProject(dir: string): Promise<void> {
    await runCliInProcess(["init", "--yes"], dir, testEnv());
  }
  ...
  it("flat .md file in backlog is listed by status", async () => {
    ...
    const output = await runCliOutputInProcess(["status"], ctx.dir, testEnv());
    expect(output).toContain("my-flat-plan.md");
  });

  it("slug-folder plan in backlog is NOT discovered (flat-only)", async () => {
    await initProject(ctx.dir);
    const bd = backlogDir(ctx.dir);
  ...
  it("multiple flat plans are listed together", async () => {
    await initProject(ctx.dir);
    const bd = backlogDir(ctx.dir);
  ...
  it("init --yes creates sample plan in global backlog", async () => {
    await initProject(ctx.dir);
    const bd = backlogDir(ctx.dir);
    ...
    expect(existsSync(join(bd, "hello-world", "hello-world.md"))).toBe(false);
  });

  it("doctor detects flat backlog plans", async () => {
    await initProject(ctx.dir);
    const bd = backlogDir(ctx.dir);
    // Add a flat plan (hello-world.md already exists from init)
    writeFileSync(join(bd, "doc-plan.md"), "# Plan: Doc Plan\n");

    const output = await runCliOutputInProcess(["doctor"], ctx.dir, testEnv());
...
describe("flat backlog plan dependencies in status", () => {
  ...
  it("shows dependency info for flat plans with depends-on frontmatter", async () => {
...
describe.skipIf(process.platform === "win32")(
  ...
  () => {
    ...
    it("dry-run detects flat backlog plan and shows promote message", async () => {
      ...
      await runCliInProcess(["init", "--yes"], testDir, env);
      ...
      const result = await runCliInProcess(["run", "--dry-run"], testDir, {
