...
/** Content for the hello-ralphai sample plan. Used by `init` and `seed`. */
const HELLO_RALPHAI_PLAN = `# Plan: Hello Ralphai
...
- [ ] A script named \`hello-ralphai.mjs\` exists in the repository root
- [ ] Running \`node hello-ralphai.mjs\` prints a greeting that mentions Ralphai
- [ ] The change is committed with a conventional commit message

## Implementation Tasks

### Task 1: Create hello-ralphai.mjs

Create a file called \`hello-ralphai.mjs\` in the repository root.

#### 1.1: Print a greeting

- Log a friendly greeting to the console (e.g. "Hello from Ralphai!")

#### 1.2: Print metadata

- Log a short note that this script was generated by Ralphai as a sample task
- Log the current date using \`new Date().toISOString()\`
`;

const HELLO_RALPHAI_SLUG = "hello-ralphai";
...
`;
  ...
  if (answers.createSamplePlan) {
    ...
    const samplePlanPath = join(backlogDir, `${HELLO_RALPHAI_SLUG}.md`);
    if (!existsSync(samplePlanPath)) {
      writeFileSync(samplePlanPath, HELLO_RALPHAI_PLAN);
...
function runSeed(cwd: string): void {
  ...
  const ipSlugDir = join(inProgressDir, HELLO_RALPHAI_SLUG);
  ...
  if (existsSync(pidPath)) {
    ...
    if (!isNaN(pid)) {
      try {
        ...
        console.log(
          `${TEXT}Cannot seed: a runner is actively executing ${HELLO_RALPHAI_SLUG} (PID ${pid}).${RESET}`,
        );
  ...
  // Clean up any existing hello-ralphai artifacts
  ...
  const archiveSlugDir = join(archiveDir, HELLO_RALPHAI_SLUG);
  if (existsSync(archiveSlugDir)) {
    rmSync(archiveSlugDir, { recursive: true, force: true });
    cleaned.push("archive");
  }

  const backlogFile = join(backlogDir, `${HELLO_RALPHAI_SLUG}.md`);
  ...
  mkdirSync(backlogDir, { recursive: true });
  writeFileSync(backlogFile, HELLO_RALPHAI_PLAN);

  if (cleaned.length > 0) {
    console.log(
      `${TEXT}Cleaned ${HELLO_RALPHAI_SLUG} from: ${cleaned.join(", ")}${RESET}`,
    );
  }
  console.log(`${TEXT}Seeded ${HELLO_RALPHAI_SLUG} into backlog.${RESET}`);
}
