...
`;
  ...
  if (answers.createSamplePlan) {
    ...
    const samplePlanPath = join(backlogDir, "hello-ralphai.md");
    if (!existsSync(samplePlanPath)) {
      const samplePlan = `# Plan: Hello Ralphai
...
- The current date
`;
      writeFileSync(samplePlanPath, samplePlan);
  ...
  if (samplePlanCreated) {
    console.log(
      `  hello-ralphai.md           ${DIM}Sample plan in backlog${RESET}`,
    );
  }
  console.log();
