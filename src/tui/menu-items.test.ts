 ...
 *
 * Ported from src/interactive/menu.test.ts with adaptations:
...
describe("buildMenuItems", () => {
  ...
  it("uses renamed group names (START, MANAGE, TOOLS)", () => {
    const state = makeState({ backlog: makeBacklog(1) });
    const items = buildMenuItems(state, NO_GITHUB);
    ...
    expect(groups).not.toContain("maintenance");
  });
});
...
describe("buildMenuItems — hotkeys", () => {
  it("assigns expected hotkeys to all items that have them", () => {
    const state = makeState({ backlog: makeBacklog(1) });
    const items = buildMenuItems(state, NO_GITHUB);

    const hotkeyMap = new Map(
      items.filter((i) => i.hotkey).map((i) => [i.value, i.hotkey]),
    );

    expect(hotkeyMap.get("resume-stalled")).toBe("r");
    expect(hotkeyMap.get("run-next")).toBe("n");
    expect(hotkeyMap.get("pick-from-backlog")).toBe("b");
    expect(hotkeyMap.get("pick-from-github")).toBe("g");
    expect(hotkeyMap.get("run-with-options")).toBe("o");
    expect(hotkeyMap.get("stop-running")).toBe("s");
    expect(hotkeyMap.get("reset-plan")).toBe("e");
    expect(hotkeyMap.get("view-status")).toBe("p");
    expect(hotkeyMap.get("doctor")).toBe("d");
    expect(hotkeyMap.get("clean")).toBe("c");
    expect(hotkeyMap.get("quit")).toBe("q");
  });
  ...
  it("all hotkeys are unique single characters", () => {
    ...
    const hotkeys = items.filter((i) => i.hotkey).map((i) => i.hotkey!);
...
// ---------------------------------------------------------------------------
// buildMenuItems — TOOLS items (replaces maintenance)
// ---------------------------------------------------------------------------

describe("buildMenuItems — TOOLS items", () => {
  it("doctor, clean, settings are in the TOOLS group and never disabled", () => {
    const state = makeState();
    const items = buildMenuItems(state, NO_GITHUB);

    for (const value of ["doctor", "clean", "settings"]) {
  ...
  it("settings item has consolidated hint", () => {
    const state = makeState();
    const items = buildMenuItems(state, NO_GITHUB);
    const settings = items.find((i) => i.value === "settings")!;
